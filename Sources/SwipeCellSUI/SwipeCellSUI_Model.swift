...
public struct SwipeCellActionItem: Identifiable {
   ...
   // public var swipeOutButtonViewScaleFactor: CGFloat
    ...
    Initializer
     ...
     - Parameter  buttonWidth: Width of the button. The open side menu width is calculated from the sum of all button widths. Default is 75.
    ...
    */
    public init(id: String = UUID().uuidString, buttonView: @escaping ()->AnyView, swipeOutButtonView: (()->AnyView)? = nil, buttonWidth: CGFloat = 70, backgroundColor: Color, swipeOutAction: Bool = false, swipeOutHapticFeedbackType: UINotificationFeedbackGenerator.FeedbackType? = nil, swipeOutIsDestructive: Bool = true, actionCallback: @escaping ()->() ){
        self.id = id
